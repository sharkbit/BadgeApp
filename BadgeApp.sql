...
DROP TABLE IF EXISTS `violation_status`;
CREATE TABLE `violation_status` (
  `id` int NOT NULL AUTO_INCREMENT,
  `badge_number` int NOT NULL,
  `violation_count` int NOT NULL DEFAULT 0,
  `status` enum('active','warning','escalated','blocked') NOT NULL DEFAULT 'active',
  `last_violation_date` datetime DEFAULT NULL,
  `blocked_until` datetime DEFAULT NULL,
  `admin_contact_required` tinyint(1) NOT NULL DEFAULT 0,
  `created_at` datetime NOT NULL,
  `updated_at` datetime NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `badge_number` (`badge_number`),
  CONSTRAINT `fk_violation_status_badge` FOREIGN KEY (`badge_number`) REFERENCES `badges` (`badge_number`) ON DELETE CASCADE
