...
class AgcCalSearch extends AgcCal {
	...
	yii::$app->controller->createLog(true, 'trexKeyWords','yesss');
		...
		if(isset($this->active)) { $query->andFilterWhere(['cal_calendar.active'=>$this->active]); }
		...
		if(isset($this->event_status_id)) { $query->andFilterWhere(['cal_event_status.event_status_id'=>$this->event_status_id]); }
