	...
	public function getSRdata($mydate,$sumary=true) {
		if ($sumary) {
			...
			$sql="SELECT c.club_name,c.short_name,c.club_id,c.is_club, ".
				...
				"(SELECT SUM( (select count(*) FROM BadgeDB.event_attendee WHERE event_attendee.ea_calendar_id=view_events.ea_calendar_id)) as `ea_sum` ".
					"FROM BadgeDB.view_events where ".$this->getWhere($mydate,'event_date')." and view_events.club_id=c.club_id group by view_events.club_id ) as students ".
