			...
			<?= GridView::widget([
				...
				'columns' => [
					...
					[   'attribute'=>'act_active',
						'value' => function($model, $attribute){ if($model->act_active==0) {return '-';} else {return 'Yes';} },
						'filter' => \yii\helpers\Html::activeDropDownList($searchModel, 'act_active', ['0' => 'No', '1' => 'Yes'],['class'=>'form-control','prompt' => 'All']),
					],
					[   'attribute' => 'act_login',
						'value' => function($model) { if($model->act_login==0) {return '-';} else {return 'Yes';} },
					...
					[   'attribute' => 'act_signup',
						'value' => function($model) { if($model->act_signup==0) {return '-';} else {return 'Yes';} },
