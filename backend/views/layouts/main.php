...
use backend\assets\GritterAsset;

AppAsset::register($this);
FontAwesomeAsset::register($this);
Select2Asset::register($this);
GritterAsset::register($this);
...
<body ng-app="agc" class="waitMe_body" id="wait-me">
    ...
    $srv_name = explode('.', $_SERVER['HTTP_HOST'])[0];
    $this->beginBody() ?>
    ...
    <div class="wrap">
        ...
        <div class="container">
            ...
            <?= \backend\widgets\AlertGritterWidget::widget() ?>
            <?= Alert::widget() ?>
            <?= $content ?>
                    ...
                    jQuery.ajax({
                        ...
                        success: function(responseData, textStatus, jqXHR) {
                            if (responseData) {
                                ...
                                $("#cred_xfer-to_badge_name").val('Not Found');
                            }
                        },
                        error: function(responseData, textStatus, errorThrown) {
                            console.log("Error:" + responseData);
        ...
        ?>
            ...
            app.controller('ViolationsRecFrom', function($scope) {
                $(document).ready(function(e) {
                    $("#violations-vi_rules")
                        .select2({
                            placeholder_text_multiple: 'Select Violations',
                            width: "100%",
                        })
                        .change(function() {
                            ...
                            if (cur_rules) {
                                if (cur_rules.indexOf(",") > 0) {
                                    ...
                                    for (var i = 0; i < arrayLength; i++) {
                                        if (spl_rules[i].slice(-1) > lvl) {
                                            lvl = spl_rules[i].slice(-1);
                                        };
                                    }
                    ...
                    if (rep_badge) {
                        getReporterName(rep_badge, 'reporter_name');
                    }

                    var rep_badge = $("#violations-badge_involved").val();
                    if (rep_badge) {
                        getReporterName(rep_badge, 'involved_name');
                    }

                    var rep_badge = $("#violations-badge_witness").val();
                    if (rep_badge) {
                        getReporterName(rep_badge, 'witness_name');
                    }
                    ...
                    jQuery.ajax({
                        ...
                        success: function(responseData, textStatus, jqXHR) {
                            ...
                            if (responseData.badge_number) {
                                ...
                                $("#violations-" + field_name).val('Not Found');
                            }
                        },
                        error: function(responseData, textStatus, errorThrown) {
                            console.log(responseData);
